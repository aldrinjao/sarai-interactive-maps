...
export class EviMapService {
  ...
  getTimeSeriesByLatLngEndpoint(coords: L.LatLngLiteral, startDate: string, endDate: string, format?: string): string {
    let endpoint = `${this._config.evi_maps.eeApiEndpoint}/time-series/${coords.lat}/${coords.lng}/${startDate}/${endDate}`;
  ...
  getDOYByLatLngEndpoint(coords: L.LatLngLiteral, startDate: string, endDate: string, format?: string): string {
    let endpoint = `${this._config.evi_maps.eeApiEndpoint}/day-of-the-year/${coords.lat}/${coords.lng}/${startDate}/${endDate}`;
  ...
  getEviTimeSeriesByLatLng(coords: L.LatLngLiteral, startDate: string, endDate: string): Observable<any> {
    ...
    return this._http
      ...
      .mergeMap((data: any) => {
        const isEmpty: boolean = every(data.result, (item: any) => {
          return item.evi !== null;
        });

        // throw a new observable containing the error message when all evi value are null
        if (isEmpty === false) {
