...
@Component({
  selector: 'app-evi-maps',
  templateUrl: './evi-maps.component.html',
  styleUrls: ['./evi-maps.component.sass']
})
export class EviMapsComponent implements OnDestroy, OnInit {
  public chartData: Observable<any>;
  private _pageTitle = 'EVI Maps';
  private _map: L.Map;
  ...
  constructor(
    ...
    private _eviMapService: EviMapService,
  ...
  setupMapClick(targetEl: HTMLElement) {
    ...
    this._popupEventListeners.push(delegate(targetEl, 'click', '.link--evi-time-series', (evt: Event) => {
      const markerPos: L.LatLng = this._marker.getLatLng();
    ...
    this._popupEventListeners.push(delegate(targetEl, 'click', '.link--evi-doy', (evt: Event) => {
      const markerPos: L.LatLng = this._marker.getLatLng();
  ...
  onMapClick(evt: Event) {
    ...
    this._marker
      .bindPopup(this.generatePopupHtml((evt as any).latlng), {
        className: 'leaflet-popup--evi leaflet-popup--with-footer-controls'
      })
      .openPopup()
      ;
  }
  ...
  showTimeSeriesChart(coords: L.LatLngLiteral, startDate: string, endDate: string, changed = true) {
    ...
    const endpoint = this._eviMapService.getTimeSeriesByLatLngEndpoint(coords, startDate, endDate, 'csv');
    ...
    if (changed === false && typeof this._oldTimeSeriesData !== 'undefined') {
      ...
      dataObservable = this._eviMapService
        .getEviTimeSeriesByLatLng(coords, startDate, endDate)
        .map((data: any) => {
    ...
    dataObservable
      .delay(300)
      .map((data: any) => {
        // extract the time and evi into separate properties
      ...
      .subscribe((data: Chart.LinearChartData) => {
        ...
        const modalTitle = `EVI Time Series Data (${parsedStartDate.format('MMMM D, YYYY')} to ${parsedEndDate.format('MMMM D, YYYY')}) \
  ...
  showDayOfTheYearChart(coords: L.LatLngLiteral, startDate: string, endDate: string, changed = true) {
    ...
    // assemble endpoint for download link
    const endpoint = this._eviMapService.getDOYByLatLngEndpoint(coords, startDate, endDate, 'csv');
    ...
    dataObservable
      ...
      .map((data: any) => {
        ...
        result.datasets = reduce(data.result, (yearlyData: any, value: any, key: number) => {
          ...
          yearlyData[year].values.push(value['evi']);
      ...
      .subscribe((data: Chart.LinearChartData) => {
        ...
        const modalTitle = `EVI Day of the Year Data (${parsedStartDate.format('MMMM D, YYYY')} to \
  ...
  processData(startDate: string, endDate: string, place?: string) {
    ...
    this._tileLayerService
      .getEviLayerData(startDate, endDate, place)
      .then((response: any) => {
        this._loader.show();
        ...
        const layer: Layer = {
          ...
          url: tileUrl,
          type: 'evi',
          layerOptions: this._tileLayerService.getEviLayerOptions()
        };
      ...
      .catch((error) => {
        ...
        this._logger.log('EVI Map Data Unavailable', message, true);
      })
      ;
  ...
  generatePopupHtml(coords: L.LatLngLiteral): string {
    return `<dl class="list list--feature-info">
      ...
      <dt class="list__item list__item--key">EVI Time Series Chart:</dt>
      <dd class="list__item list__item--value">
          <a href="#" class="link link--evi-time-series">
            <i class="fa fa-line-chart link__icon" aria-hidden="true"></i>
            <span class="link__text">Show</span>
          </a>
      </dd>

      <dt class="list__item list__item--key">EVI Day of the Year Chart:</dt>
      <dd class="list__item list__item--value">
          <a href="#" class="link link--evi-doy">
            <i class="fa fa-line-chart link__icon" aria-hidden="true"></i>
            <span class="link__text">Show</span>
          </a>
      </dd>
